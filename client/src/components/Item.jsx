  ...
  return (
    <div className="overflow-hidden w-full transition-all duration-300 hover:-translate-y-1 cursor-pointer group/item">
      ...
      <div
        ...
        className="flexCenter h-[140px] sm:h-[160px] md:h-[180px] lg:h-[200px] xl:h-[220px] w-full transition-all duration-300 rounded-lg sm:rounded-xl group relative overflow-hidden shadow-sm hover:shadow-md"
      ...
      >
        <img
          ...
          className="w-auto h-full object-contain p-2 md:p-3 lg:p-3 xl:p-4 transition-transform duration-300 group-hover:scale-105"
        />
      ...
      {/*info */}
      <div className="pt-2 sm:pt-3 md:pt-3 lg:pt-4 xl:pt-4 px-0.5 sm:p-1 md:p-1.5">
        {/*title and description */}
        <div className="flexBetween gap-1 sm:gap-2 md:gap-2 lg:gap-3">
          <h5 className="text-xs sm:text-sm md:text-base lg:text-base xl:text-base font-semibold uppercase line-clamp-1 flex-1 min-w-0 hover:text-tertiary transition-colors cursor-pointer">{product.title}</h5>
          <p className="text-xs sm:text-sm md:text-base lg:text-base xl:text-base uppercase font-bold whitespace-nowrap flex-shrink-0 text-tertiary">
            {currency}{getPrice()}
          </p>
        </div>
        <p className="text-[10px] sm:text-xs md:text-sm lg:text-sm xl:text-sm text-gray-600 line-clamp-2 pt-1 sm:pt-1.5 md:pt-2 leading-relaxed">{product.description}</p>
      </div>
    </div>
  );
};
